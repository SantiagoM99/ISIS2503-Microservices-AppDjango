...
def check_place(data):
    r = requests.get(settings.PATH_PS, headers={"Accept":"application/json"})
    places = r.json()
    for place in places:
...
def MeasurementCreate(request):
    if request.method == 'POST':
        ...
        if check_variable(data_json) == True and check_place(data_json) == True:
            ...
            return HttpResponse("unsuccessfully created measurement. Place or Variable does not exist")
